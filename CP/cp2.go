...
func main() {
    ...
    for{
        ...
        fmt.Println("Main Mutex Lock", runtime.NumGoroutine(), step_no-s_no, b_flag, cp_bcast, m_flag)
    ...
    if cp_bcast == cp_no && b_flag == true {
        ...
        if conn := acceptConnections(cp_no, sock); conn != nil { //If Data is available
            ...
            fmt.Println("Handle Client No. of goroutines", runtime.NumGoroutine(), parseCommonName(conn))
...
func broadcastData(step_no uint32, src int, data []byte) {
    ...
    for i := 0; i < no_CPs; i++ {
        ...
        if i + 1 != src {
            ...
            go func(sign_msg []byte, src int, dst int) {
                ...
                fmt.Println("Broadcast START No. of goroutines", runtime.NumGoroutine())

                defer wg.Done() //Decrement WaitGroup counter

                sendDataToDest(sign_msg, src, dst)
                fmt.Println("Broadcast No. of goroutines & CP_bcast", runtime.NumGoroutine(), src)
    ...
    fmt.Println("Broadcast END No. of goroutines", runtime.NumGoroutine())
}
...
func sendDataN_1(step_no uint32, src int, cp int, data []byte) {
    ...
    for i := 0; i < no_CPs; i++ {
        ...
        if i + 1 != cp && i + 1 != src {
            ...
            go func(sign_msg []byte, src int, dst int) {
                ...
                fmt.Println("Send data n-1 START No. of goroutines", runtime.NumGoroutine())

                defer wg.Done() //Decrement WaitGroup counter

                sendDataToDest(sign_msg, src, dst)
                fmt.Println("Send data n-1 No. of goroutines & CP_bcast", runtime.NumGoroutine(), src)
    ...
    wg.Wait() //Wait for data to be sent to all CPs
    fmt.Println("Send data n-1 END No. of goroutines", runtime.NumGoroutine())
}
